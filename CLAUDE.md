...
| 1.0.47 | 62 | 審査提出済み | 大会エントリーの承認待ちドラフトに個別メンバー取り消し・追加招待機能を追加・主催者/編集者向けに承認待ちチーム一覧を表示・間違えて辞退した場合に参加へ戻せるボタンを追加・辞退した人を自動除外して手動取り消しを忘れても成立するように修正 |
...
### リリース状況
- **iOS**: 🟢 1.0.46 リリース済み（起動時ポップアップに大会名・日付追加・公式アカウントのフォロー数表示・管理者用フォロー操作・メンバー検索表記ゆれ対応・チーム参加リクエストのポップアップ統合・通知用メールアドレス機能）
- **Android**: 🟢 1.0.46 リリース済み（製品版公開済み・Google Play）
