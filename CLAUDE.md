...
| 1.0.50 | 68 | iOS: TestFlight実機確認中 / Android: 審査提出済み | 1.0.50+67 に追加で「あなた宛」タブのエントリー招待・募集開始カードに送信者アバター（＋種類バッジ）を表示（システム発以外の通知全般。名前の太字差し込みはentry_inviteのみ）。iOSはTestFlightで実機確認中（OKなら1.0.50+69で審査提出）、AndroidはTestFlightの仕組みが無いため2026-09-26に先行してGoogle Play製品版へ審査提出（iOSの確認結果を待たず独立提出） |
