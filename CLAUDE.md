...
| 1.0.50 | 67 | TestFlight のみ | 実機確認用。ホームタブに未読バッジ（ベル通知＋お知らせ合計）・エントリー招待通知タップで大会詳細へ遷移・「あなた宛」タブにもエントリー招待を表示・承認待ちエントリーカードを折りたたみ式に（大会詳細が隠れる問題を解消）・お知らせ配信画面に「大会を選んでリンクを自動入力」・大会名が空欄になる通知バグ修正（Functions側は提出不要で反映済み）。OKなら 1.0.50+68 で審査提出 |
...
- **Android**: 🟢 1.0.48 リリース済み（製品版公開済み・Google Play）
